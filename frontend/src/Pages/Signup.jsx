...
export default function Signup() {
  ...
  return (
    <div className='w-full min-h-screen bg-cover flex items-center justify-center p-4 sm:p-6 lg:p-8' style={{backgroundImage:`url('https://wallpapers.com/images/featured/iron-man-superhero-ponky3hlfivddo2m.jpg')`}}>
      ...
      <form onSubmit={handleSignup} className="relative w-full max-w-sm sm:max-w-md border bg-black/20 backdrop-blur-xl border-white/30 rounded-3xl shadow-2xl p-6 sm:p-8 lg:p-10 space-y-4">
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-8 text-center">Register</h1>
        ...
        <button type="submit" className="w-full bg-linear-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-bold py-3 px-4 rounded-xl transition duration-300 shadow-lg hover:shadow-xl mt-6" disabled={loading}>{loading ? "loading..." : "Sign Up"}</button>
        <p className="text-white/60 ml-2 text-center">Already have an account? <span className="text-blue-500 underline cursor-pointer" onClick={()=>navigate("/login")}>Log in</span></p>
      </form>
    </div>
  )
}
